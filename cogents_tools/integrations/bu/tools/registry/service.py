...
class Registry(Generic[Context]):
    ...
    def _normalize_action_function_signature(
        ...
        async def normalized_wrapper(*args, params: BaseModel | None = None, **kwargs):
            ...
            if args:
                raise TypeError(
                    f"{func.__name__}() does not accept positional arguments, only keyword arguments are allowed"
                )
    ...
    async def execute_action(
        ...
        try:
            ...
            if "requires browser_session but none provided" in str(
                e
    ...
    def create_action_model(
        self, include_actions: list[str] | None = None, page_url: str | None = None
    ) -> type[ActionModel]:
