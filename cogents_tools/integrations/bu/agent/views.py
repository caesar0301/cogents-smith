...
from cogents_tools.integrations.bu.agent.message_manager.views import MessageManagerState
from cogents_tools.integrations.bu.browser.views import BrowserStateHistory
from cogents_tools.integrations.bu.dom.views import DEFAULT_INCLUDE_ATTRIBUTES, DOMInteractedElement, DOMSelectorMap

# from cogents_tools.integrations.bu.dom.history_tree_processor.service import (
# 	DOMElementNode,
# 	DOMHistoryElement,
...
class ActionResult(BaseModel):
    ...
    include_extracted_content_only_once: bool = (
        False  # Whether the extracted content should be used to update the read_state
    )
...
class AgentHistory(BaseModel):
    ...
    def get_interacted_element(
        model_output: AgentOutput, selector_map: DOMSelectorMap
    ) -> list[DOMInteractedElement | None]:
        elements = []
...
class AgentHistoryList(BaseModel, Generic[AgentStructuredOutput]):
    ...
    # 	"""
    # 	from cogents_tools.integrations.bu.agent.playwright_script_generator import PlaywrightScriptGenerator
