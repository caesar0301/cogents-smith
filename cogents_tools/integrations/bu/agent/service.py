...
from cogents_tools.integrations.bu import Browser, BrowserProfile, BrowserSession

# Lazy import for gif to avoid heavy agent.views import at startup
# from cogents_tools.integrations.bu.agent.gif import create_history_gif
from cogents_tools.integrations.bu.agent.message_manager.service import MessageManager
from cogents_tools.integrations.bu.agent.prompts import SystemPrompt
from cogents_tools.integrations.bu.agent.views import (
    ActionResult,
    AgentError,
    AgentHistory,
    AgentHistoryList,
...
from cogents_tools.integrations.bu.utils import (
    _log_pretty_path,
...
class Agent(Generic[Context, AgentStructuredOutput]):
    ...
    def logger(self) -> logging.Logger:
        ...
        return logging.getLogger(
            f"cogents_tools.integrations.bu.Agent🅰 {self.task_id[-4:]} ⇢ 🅑 {_browser_session_id[-4:]} 🅣 {_current_target_id}"
        )
    ...
    def _set_screenshot_service(self) -> None:
        ...
        try:
            from cogents_tools.integrations.bu.screenshots.service import ScreenshotService
    ...
    async def _execute_actions(self) -> None:
        ...
        self.logger.debug(
            f"⚡ Step {self.state.n_steps}: Executing {len(self.state.last_model_output.action)} actions..."
        )
    ...
    async def _finalize(self, browser_state_summary: BrowserStateSummary | None) -> None:
        ...
        if browser_state_summary:
            ...
            await self._make_history_item(
                self.state.last_model_output, browser_state_summary, self.state.last_result, metadata
            )
    ...
    def _log_agent_event(self, max_steps: int, agent_run_error: str | None = None) -> None:
        ...
        json.dumps(final_res) if final_res is not None else None
    ...
    async def run(
        ...
        from cogents_tools.integrations.bu.utils import SignalHandler
        ...
        try:
            ...
            from cogents_tools.integrations.bu.browser.events import BrowserStartEvent
            ...
            if self.browser_session and self.browser_session.agent_focus:
                self.logger.debug(
                    f"🎯 Browser focus established on target: {self.browser_session.agent_focus.target_id[-4:]}"
                )
            else:
        ...
        for i, action in enumerate(actions):
            if i > 0:
                ...
                if action.model_dump(exclude_unset=True).get("done") is not None:
                    msg = (
                        f"Done action is allowed only as a single action - stopped after action {i} / {total_actions}."
                    )
